...
// Fix: Removed PhoneInput which does not exist in ui.tsx
import { Button, Input, Heading, Text, Select, Badge } from '../components/ui';
import { ArrowRight, Star, Shield, Briefcase, Zap, Target, Landmark, Sparkles, Mail, Phone, Percent, UserCheck, Scale, Building, UserCircle, Users } from 'lucide-react';
...
export const Onboarding = ({ user, onComplete }: { user: User, onComplete: () => void }) => {
  ...
  const executeCompletion = () => {
    const updates: Partial<User> = {
      ...
      hasAgentFee: formData.hasAgentFee,
      ...
      <div className="w-full max-w-4xl space-y-16 relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
        ...
        {step === 1 && (
          <div className="text-center space-y-12">
            ...
            <div className="pt-16">
              <Button onClick={handleNext} className="h-14 px-10 text-[11px] tracking-[0.6em] font-black">Report for Duty</Button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-12 animate-in slide-in-from-right duration-500">
             ...
             <div className="grid md:grid-cols-2 gap-2">
                ...
                <div className="space-y-3">
                   <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white flex items-center gap-2">
                ...
                <div className="space-y-3">
                   ...
                   {/* Fix: Replaced PhoneInput with Input */}
                ...
                <div className="md:col-span-2 space-y-3 pt-6">
                   <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white">Base of Operations</label>
             ...
             <div className="flex justify-between pt-20 border-t border-white/5">
               ...
               <Button onClick={handleNext} className="h-14 px-10 text-[10px] tracking-[0.4em] font-black" disabled={!formData.firstName || !formData.lastName || !formData.email}>Next Setup <ArrowRight size={14} className="ml-4" /></Button>
             </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-12 animate-in slide-in-from-right duration-500">
             ...
             <div className="p-10 bg-accent/5 border border-accent/20 glass-ui mb-8 flex items-start gap-8">
                ...
                <div className="space-y-3">
                   <p className="text-[11px] font-black uppercase tracking-[0.5em] text-accent">Platform Intelligence Note</p>
                   <p className="text-lg text-white font-light italic leading-relaxed">
                     Based on your marks (<b>{formData.selectedRoles.join(', ')}</b>), the Platform is preparing trajectories for the following guilds.
                   </p>
                </div>
             </div>
                ...
                {UNIONS.map(u => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white leading-relaxed">{u.description}</span>
                    </button>
                  );
                })}
             </div>
             ...
             <div className="flex justify-between pt-20 border-t border-white/5">
               <button onClick={handleBack} className="text-[10px] font-black uppercase tracking-[0.5em] text-white hover:underline transition-colors">Back to Ones</button>
               <Button onClick={handleNext} className="h-14 px-10 text-[10px] tracking-[0.4em] font-black" disabled={formData.selectedUnions.length === 0}>Establish Objectives <ArrowRight size={14} className="ml-4" /></Button>
             </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-12 animate-in slide-in-from-right duration-500">
             ...
             <div className="grid md:grid-cols-2 gap-2">
                {[
                  ...
                  >
                    ...
                    <span className="text-[11px] font-black uppercase tracking-[0.5em] text-white">{goal.label}</span>
                  </button>
                ))}
             </div>
             ...
             <div className="flex justify-between pt-20 border-t border-white/5">
               <button onClick={handleBack} className="text-[10px] font-black uppercase tracking-[0.5em] text-white hover:underline transition-colors">Back to Ones</button>
               <Button onClick={handleNext} className="h-14 px-10 text-[10px] tracking-[0.4em] font-black">Fiscal Entity <ArrowRight size={14} className="ml-4" /></Button>
             </div>
          </div>
        )}
        ...
        {step === 6 && (
          <div className="space-y-12 animate-in slide-in-from-right duration-500">
             <div className="space-y-2">
                <Text variant="caption">Scene 05 // Fiscal Identity</Text>
                <Heading level={2} className="text-white">The Structure.</Heading>
             </div>

             <div className="p-10 bg-accent/5 border border-accent/20 glass-ui mb-8 flex items-start gap-8">
                <Scale className="text-accent shrink-0" size={28} />
                <div className="space-y-3">
                   <p className="text-[11px] font-black uppercase tracking-[0.5em] text-accent">Tax Protocol Calibration</p>
                   <p className="text-lg text-white font-light italic leading-relaxed">
                     Billing as a {formData.businessStructure?.replace('_', ' ')} in <b>{formData.province}</b> requires specific ledger logic. Platform details for this structure are coming soon.
                   </p>
                </div>
             </div>
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-1">
                {[
                  { id: 'INCORPORATED', label: 'Incorporated', icon: Building },
                  { id: 'SOLE_PROPRIETORSHIP', label: 'Sole Prop', icon: UserCircle },
                  { id: 'EMPLOYEE', label: 'Employee', icon: Briefcase },
                  { id: 'ORGANIZATION', label: 'Org / Studio', icon: Users }
                ].map((struct) => (
                  <button 
                    key={struct.id}
                    onClick={() => setFormData({...formData, businessStructure: struct.id as any})}
                    className={clsx(
                      "p-8 border transition-all duration-700 flex flex-col items-center text-center gap-6 h-48 justify-center", 
                      formData.businessStructure === struct.id ? "bg-accent text-black border-accent" : "glass-ui text-white hover:border-white/20"
                    )}
                  >
                    <struct.icon size={24} strokeWidth={1.5} />
             ...
             <div className="flex justify-between pt-20 border-t border-white/5">
               <button onClick={handleBack} className="text-[10px] font-black uppercase tracking-[0.5em] text-white hover:underline transition-colors">Back to Goals</button>
               <Button onClick={handleNext} className="h-14 px-10 text-[10px] tracking-[0.4em] font-black">Configure Commission <ArrowRight size={14} className="ml-4" /></Button>
             </div>
          </div>
        )}
        ...
        {step === 7 && (
          <div className="space-y-12 animate-in slide-in-from-right duration-500">
             <div className="space-y-2">
                <Text variant="caption">Scene 06 // Agency Protocol</Text>
                <Heading level={2} className="text-white">The Commission.</Heading>
             </div>

             <div className="grid md:grid-cols-1 gap-12">
                <button 
                  onClick={() => setFormData({...formData, hasAgentFee: !formData.hasAgentFee})}
                  className={clsx(
                    "p-12 border transition-all duration-700 flex items-center gap-8 text-left", 
                    formData.hasAgentFee ? "bg-accent text-black border-accent" : "glass-ui text-white hover:border-white/20"
                  )}
                >
                  <div className={clsx("w-12 h-12 flex items-center justify-center border", formData.hasAgentFee ? "border-black" : "border-white/20")}>
                    {formData.hasAgentFee ? <UserCheck size={24} /> : <div className="w-4 h-4 rounded-full border border-white/40" />}
                  </div>
                  <div>
                    <span className="text-3xl font-serif italic block">Agency Representation</span>
                    <span className="text-[10px] font-black uppercase tracking-widest opacity-60">I have an agent who collects a commission on my bookings.</span>
                  </div>
                </button>

                {formData.hasAgentFee && (
                  <div className="animate-in slide-in-from-top-4 duration-500 space-y-8 p-12 glass-ui border-accent/20">
                    <div className="space-y-6">
                       <label className="text-[11px] font-black uppercase tracking-[0.5em] text-accent flex items-center gap-4">
                       ...
                       <div className="flex items-center gap-6">
                          ...
                          <div className="space-y-1">
                             <p className="text-4xl font-serif text-white italic">%</p>
                             <p className="text-[9px] font-black uppercase tracking-widest text-white/40">Default Logic</p>
                          </div>
                       </div>
                    </div>
                    <p className="text-xs text-white/60 leading-relaxed font-light italic">
                      This rate will be automatically deducted from your production logs to provide an accurate "Net Scale" view in your <b>Wrap Wallet</b>.
                    </p>
                  </div>
                )}
             </div>
             ...
             <div className="flex justify-between pt-20 border-t border-white/5">
               <button onClick={handleBack} className="text-[10px] font-black uppercase tracking-[0.5em] text-white hover:underline transition-colors">Adjust Structure</button>
               <Button onClick={handleFinish} className="h-14 px-10 bg-white text-black font-black uppercase tracking-[0.6em] text-[11px]">Print It // Action</Button>
             </div>
          </div>
        )}
