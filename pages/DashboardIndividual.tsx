...
import { ArrowUpRight, Shield, Landmark, Radar, BookOpen, HelpCircle, Info, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-24 animate-in fade-in duration-1000">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-12 pb-16 border-b border-white/10 relative">
        <div className="space-y-6">
          <Badge color="accent" className="uppercase tracking-widest text-[9px] px-6">Principal Dashboard</Badge>
          <h1 className="heading-huge text-white uppercase italic">THE <br /><span className="text-accent">WRAP.</span></h1>
        </div>
        <div className="flex gap-4">
          <Button variant="outline" onClick={() => navigate('/resources')} className="h-20 px-8 text-[10px] border-white/10 group">
             <BookOpen size={16} className="mr-4 text-accent" /> Field Manual
          </Button>
          <Button onClick={() => navigate('/jobs/new')} className="h-20 px-12 text-[12px] font-black tracking-[0.5em] bg-white text-black hover:bg-accent">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-1">
        <DashStat label="Guild Alignment" value={`${Math.round(eligibilityPercent)}%`} subtext={primaryTrack ? `${primaryTrack.unionName} Trajectory` : 'No Targets'} highlight />
        <DashStat label="YTD Scale" value={`$${((financeStats?.grossIncomeYTD || 0) / 1000).toFixed(1)}k`} subtext="Professional Yield" />
        <DashStat label="Compliance Hub" value="SECURE" subtext="Verified Proofs" />
        <DashStat label="GST Threshold" value={`${Math.round(gstProgress)}%`} subtext="Rule GST-001 Sync" />
      </div>
        ...
        <section className="lg:col-span-2 glass-ui p-12 space-y-12">
          <div className="flex items-center justify-between border-b border-white/5 pb-8">
            <div className="flex items-center gap-6">
               ...
               <h3 className="font-serif italic text-4xl text-white">Target Tracking</h3>
            </div>
            <div className="flex items-center gap-4 text-[9px] font-black uppercase text-white/20 italic cursor-help" title="View Registry Descriptions">
               Requirements <HelpCircle size={12} onClick={() => navigate('/resources')} />
            </div>
          </div>
        ...
        <aside className="space-y-8">
           <Card className="p-10 border-accent/20 bg-accent/5 space-y-10">
              <div className="flex items-center gap-4">
                 ...
                 <h4 className="text-[11px] font-black uppercase tracking-[0.4em] text-accent">Ontario Calibration</h4>
              </div>
              <div className="space-y-6">
                 <p className="text-sm text-white/60 italic leading-relaxed">
                    Based on your coordinates, remember that in Ontario, the **DGC** strictly covers **Set P.A.s, Art Directors, and Locations**.
                 </p>
                 <div className="p-6 bg-white/5 border border-white/5 space-y-4">
                    <span className="text-[9px] font-black uppercase tracking-widest text-white/20 flex items-center gap-2"><Info size={10} /> Local 634 Alert</span>
                    <p className="text-xs text-white/80 leading-relaxed italic">Northern Ontario/Ottawa applicants require 18 months of proven residency for membership.</p>
                 </div>
              </div>
              <Button variant="outline" className="w-full text-[9px] border-accent/20 text-accent hover:bg-accent hover:text-black" onClick={() => navigate('/resources')}>Open Manual</Button>
           </Card>
           ...
           <div className="p-8 glass-ui border-white/5 flex items-center justify-between group cursor-pointer" onClick={() => navigate('/finance')}>
              <div className="space-y-1">
                 <span className="text-[9px] font-black uppercase tracking-widest text-white/20 italic">Financial Forecast</span>
                 <p className="text-xl font-serif italic text-white">Application Fees</p>
              </div>
