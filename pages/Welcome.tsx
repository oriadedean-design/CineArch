...
export const Welcome = ({ onEnter }: { onEnter: (asAgent?: boolean) => void }) => {
  ...
  return (
    <div className="relative min-h-screen font-sans text-textPrimary bg-black overflow-x-hidden">
        ...
        <div className="text-center space-y-6 z-10">
          ...
          <div className="pt-6 animate-in fade-in duration-1000 delay-500">
            <button
              ...
              className="flex items-center gap-3 mx-auto text-[10px] font-black uppercase tracking-[0.5em] text-white/20 hover:text-accent transition-colors group"
            >
              <Eye size={12} className="group-hover:text-accent" />
              Preview UI — No Login Required
            </button>
          </div>
        </div>
      ...
      <section className="py-24 md:py-40 border-y border-white/5 bg-surface/50 backdrop-blur-3xl">
         <div className="mobile-wrapper space-y-16 md:space-y-24">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 md:gap-12">
               <div className="space-y-4 md:space-y-6 max-w-2xl">
                  <Badge color="accent">Jurisdictional Intelligence</Badge>
                  <h2 className="font-serif text-4xl md:text-7xl italic text-white leading-[0.85]">Calibrated for <br/> Your Hub.</h2>
                  <p className="text-base md:text-xl text-white/40 font-light italic leading-relaxed">CineArch recognizes the specific boundaries between technical locals. From IATSE 873 Toronto to Local 634 Northern Ontario and the DGC/ACTRA National standards. We track your eligibility in real-time.</p>
               </div>
               <Button onClick={() => navigate('/manual')} variant="outline" className="h-16 md:h-20 border-accent/20 text-accent group px-8 md:px-10 shrink-0">
                  Field Manual <ChevronRight size={14} className="ml-3 group-hover:translate-x-1 transition-transform" />
               </Button>
            </div>
               ...
               {UNIONS.map(u => (
                  <div key={u.id} className="p-6 md:p-8 glass-ui flex flex-col justify-between gap-8 group hover:border-accent/30 transition-all min-h-[200px] md:min-h-[240px]">
                     <div className="space-y-3">
                        ...
                        <span className="text-xs font-black uppercase text-white/20 tracking-wide leading-relaxed block">{u.description}</span>
                     </div>
      ...
      <section className="py-24 md:py-40 bg-black">
        <div className="mobile-wrapper grid lg:grid-cols-12 gap-12 md:gap-20 items-center">
           <div className="lg:col-span-8 space-y-8 md:space-y-12">
              <h2 className="font-serif text-4xl md:text-6xl lg:text-8xl italic text-white tracking-tighter leading-none uppercase">Authority. <br/>Precision. <br/>Discretion.</h2>
              <div className="space-y-6 md:space-y-8 text-base md:text-xl font-light text-textSecondary leading-relaxed max-w-3xl italic">
      ...
      {/* Membership Section */}
      <section className="py-24 md:py-40 bg-black/60 border-t border-white/5">
        <div className="mobile-wrapper">
          <h2 className="heading-huge text-center mb-16 md:mb-24 uppercase italic">Your Membership.</h2>
          <div className="grid md:grid-cols-3 gap-1">
             {Object.values(PLANS).map((plan) => (
               <div key={plan.id} className={clsx("p-8 md:p-12 border border-white/5 flex flex-col transition-all duration-700", plan.id === 'pro' ? "bg-accent/5 border-accent/20" : "bg-black/40")}>
                 <h3 className="text-xl md:text-2xl font-serif italic text-white mb-2">{plan.label}</h3>
                 <div className="mb-8 md:mb-12">
                    <div className="text-5xl md:text-6xl font-serif text-white italic tracking-tighter">{plan.price}<span className="text-xs font-sans text-textTertiary ml-2">/mo</span></div>
                    <p className="text-xs font-black uppercase tracking-widest text-textTertiary mt-3">{plan.desc}</p>
                 </div>
                 <ul className="space-y-4 md:space-y-6 mb-10 md:mb-16 flex-1 border-t border-white/10 pt-8 md:pt-12">
                    {plan.benefits.map((benefit, j) => (
                      <li key={j} className="flex items-start gap-3 md:gap-4 text-xs font-medium text-textSecondary italic">
                        <span className="text-accent mt-0.5 font-bold">+</span>
                        <span>{benefit}</span>
                      </li>
                    ))}
                 </ul>
                 <Button onClick={() => onEnter(plan.id === 'agency')} className={clsx("w-full h-16 md:h-20 text-[10px] tracking-[0.4em]", plan.id === 'pro' ? "bg-accent text-black" : "bg-white/10 text-white hover:bg-white")}>Select Your Tier</Button>
               </div>
             ))}
          </div>
        </div>
